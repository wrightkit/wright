...
| **Named arguments / `ChaseReeval` alias**: `chase(A, 10, rate=2, ChaseReeval.NONE)` | accept (contextual alias resolution) | reject (named args unsupported) | **implemented (#110)**: generic `name = expr` binding against the manifest signatures plus the `chase` special form (`rate=`/`duration=` keywords, `ChaseReeval` resolving only in that call context); probes `chase-keywords`, `chase-reeval-context`, `chase-keyword-binding`, the `chase-*` diagnostic probes, and the `synthetic/chase-keywords` corpus fixture (differential parity) |
...
* **Proposed decomposition.** The bounded child-issue categories (C1 to C5) and
